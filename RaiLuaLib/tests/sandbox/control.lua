...
script.on_event({'sandbox-zoom-in', 'sandbox-zoom-out', 'sandbox-zoom-in-extended', 'sandbox-zoom-out-extended'}, function(e)
  ...
  if player.render_mode == defines.render_mode.game then
    ...
    local in_map_editor = player.controller_type == defines.controllers.editor
    if e.input_name:find('in') then
      player_table.zoom = math.min(player_table.zoom * delta, ((extended or in_map_editor) and 40 or 3))
    else
      player_table.zoom = math.max(player_table.zoom / delta, ((extended or in_map_editor) and 0.1 or 0.5))
    end
